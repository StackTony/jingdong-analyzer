...
def collect(month: str | None = None, trial: bool = False) -> None:
    """启动月度抓取（A 路线手动触发）"""
    ...
    with engine.begin() as conn:
        conn.execute(Batch.__table__.insert().values(
            batch_id=batch_id,
...
def main():
    ...
    p_collect.add_argument("--trial", action="store_true", help="试爬模式（1-2 品类 × 1000 URL）")
    ...
    if args.command == "collect":
        collect(month=args.month, trial=args.trial)
