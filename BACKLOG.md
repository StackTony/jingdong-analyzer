...
- [x] **G18 连调验证**: 真实 LLM 连调发现流式不显示——根因 GLM 思考期 `delta.content=None`、内容在 `reasoning_content`（G19 修复）；短报告 <8 chunk 边缘已修（结束定格 flush）
- [x] **G19 流式 reasoning_content 透传**: 铲屎官实测「流式输出还是没有在 web 界面显示」。根因（裸 SDK 直测）：GLM 推理模型思考阶段 content 全程 None（实测 5.9-21.5s），思考内容在 reasoning_content——G18 只透 content 思考期必然空白。修法：`chat_stream` 契约升级 yield (kind, text) 双段（reasoning/content，base 回落 ('content', 全文)）；reviewer `on_delta(kind, chunk)` 双参回调（返回值只含正文不污染沉淀）；web 双区流式渲染（思考灰色区 + 正文区，节流 8 chunk）+ 结束定格 flush。真实网关连调：reasoning 5.9s 首块（737 块）、content 21.5s 起（680 块）、40.4s 全程。关羽跨家族 review 放行覆盖 cce0a58（含其 G18 review 盲点复盘：未验证真实网关 content 行为），merge commit 1d5ac6b
- [ ] **G19 验收尾巴（铲屎官体感）**: ① 思考区结束时是否残留/正文区是否稳定（st.empty 与 st.status 渲染时序，关羽记录 1）② 思考/正文节流是否分阈值（737 块/16s 思考流偏密，云长倾向思考区降 4，以体感为准）③ 长思考区是否加限高滚动容器
