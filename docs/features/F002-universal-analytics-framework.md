...
| 2026-09-03 | 两层配置直填 key + 运行进度条 G15 merged：`ai_providers.local.yaml`（gitignored `*.local.yaml`）深合并覆盖主配置；api_key 直填为主路径（`_is_placeholder_key` 识别 sk-xxx 占位符回退 env 兜底，两条都空明确报错）；`api_key_env` 值校验防填错。进度条：`run()/execute()` 加 `progress(stage, current, total, detail)` 回调（向后兼容，异常双层兜底不反噬主流程），Web `st.status` 分阶段容器 + execute 单进度条替代死转圈，CLI 逐阶段打印 stderr（feat/f002-local-conf，TDD 红绿，260 passed 无回归，关羽跨家族 review 放行） |
| 2026-09-03 | Flow Library 界面管理 G16 merged：store 层 `delete/update_template` + `delete/update_plan`（update 不新建，不存在抛 KeyError）；web 新增「Flow Library 管理」tab——模板元字段展示 + steps YAML 编辑保存（`_apply_template_edit` 从原模板回填 stability / promoted_from_plan_id 等元字段，编辑表单不接触）+ 模板/Plan 删除二次确认（session_state 确认状态机）。顺手修 test_llm fixture 隔离 bug（6 处 `_CONFIG_PATH` patch 全部配套 `_LOCAL_CONFIG_PATH` 隔离，主仓库基线 2 failed 的根因）（feat/f002-g16-flow-ui，TDD 红绿 17 新用例，277 passed 全绿，关羽跨家族 review 放行覆盖 SHA ab456d4，merge commit 1a098fe） |
| 2026-09-04 | 图表质量修复 G17 merged：铲屎官三件事根因修复——① visualizer `y=list` 逐列展开多 trace（`model.aggregate` 恒产出 list y，旧 `go.Bar(y=df[list])` 收 2D 嵌套→空图/畸形图，A 轨冷启动模板全中招；多指标只画 1 错 trace=「维度少」）② web「🔍 执行过程」默认折叠 expander（`_format_run_log` 渲染 result.log 步骤表：成败/op/图表产出/失败原因）③ fake 污染产物清理（`tpl-fake-da4d81cc` 晋升进包内置库劫持 B 轨=「错误图表」源头；证据 commit 进分支 + promoter 断根护栏测试钉死"源 plan 删则不复活"）。防再生 hot 候选（测试隔离）与晋升门槛收紧 OQ 记 BACKLOG（feat/f002-g17-chart-quality，TDD 红绿 9 新用例，286 passed 无回归，关羽跨家族 review 放行，merge commit de47fc1） |
| 2026-09-05 | 执行过程增强 + 流式 Reviewer + llm_calls 修正 G18 merged：① executor run_log 每步 entry 新增 `args`（该步用的列/参数，模板变量解析可核对）+ `shape_before/after`（数据形态变化可见）+ model 步 report 扩为 title/x/y 维度，web `_step_detail` 渲染（旧格式向后兼容）② AI Reviewer 流式三层链路：`LLMProvider.chat_stream`（base 回落 chat()，OpenAI 兼容 stream=True 逐 chunk）/ `AIReviewer.review_stream(on_delta)`（base 回落 review()）/ `run(on_review_delta=...)` 转发，web `st.empty()` 节流增量刷新——AI 报告边生成边显示不再死等 ③ reviewer 调用计入 `llm_calls`（铲屎官指出漏计：A 轨命中 + reviewer 开启时原 0 但实际烧 token；连动 test_cold_start 两处期望修正——原 0 本就是漏计）。非阻断记录：llm_calls 语义=调用次数非 token 数（FakeReviewer 也计 1，真实 token 统计 P7+ 与 provider usage 透出一起做）、短报告节流边缘、mkdtemp 清理（feat/f002-g18，TDD 红绿 20 新用例，306 passed 无回归，关羽跨家族 review 放行覆盖 SHA 830c1a7 含 review 中追加 commit 的 headRefOid 核对，merge commit 6756c65） |
| 2026-09-07 | 流式 reasoning 透传 G19 merged：G18 合入后铲屎官实测流式不显示——裸 SDK 直测根因：GLM 推理模型思考阶段 `delta.content` 全程 None（实测 5.9-21.5s，占全程一半），思考内容在 `reasoning_content`，G18 只透 content 思考期必然空白。修法三层契约升级：`chat_stream` yield `(kind, text)` 双段（reasoning/content，base 回落 ('content', 全文)，标准 OpenAI delta 无该字段 getattr 兜底）；`review_stream` 的 `on_delta(kind, chunk)` 双参回调（返回值只含正文，save_run 沉淀不被思考 token 污染）；web 双区流式渲染（思考灰色引用区 + 正文区，节流 8 chunk）+ 结束定格 flush（修 G18 短报告 <8 chunk 边缘）。真实网关连调：reasoning 5.9s 首块（737 块）、content 21.5s 起（680 块）、40.4s 全程。关羽跨家族 review 放行覆盖 cce0a58 + 诚实复盘其 G18 review 盲点（"只透 content"未要求真实网关验证）；验收尾巴 3 条（思考区残留/分阈值/限高容器）记 BACKLOG（feat/f002-g19-stream-reasoning，TDD 红绿 6 新用例 + G18 契约迁移，312 passed 无回归，merge commit 1d5ac6b） |
